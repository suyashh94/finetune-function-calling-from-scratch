...
class GPTConfig:
    ...
    is_regression: bool = False  # True: regression, False: classification
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        if config.is_regression:
            self.reg_head = nn.Linear(config.n_embd, 1, bias=False)
    ...
    def forward(
        self, idx, targets=None, prompt_idx=None, answer_idx=None, return_losses_separately=False
    ):
        device = idx.device
        b, t = idx.size()
        assert (
        ...
        if targets is not None:
            ...
            if not return_losses_separately:
                ...
                lossDict = {"prompt_loss": prompt_loss, "answer_loss": answer_loss, "loss": loss}
    ...
    def generate_answer_for_question(
        ...
        """
        Idx is the 1,t question tensor at the start which is then appended with the answer tokens
        """
        ...
        # import pdb; pdb.set_trace()
        if isTraining:
            # prune idx till the last end of prompt token
...
if __name__ == "__main__":
    GPTModel = GPT(GPTConfig())
    print("model parmas: ", GPTModel.get_num_params())
